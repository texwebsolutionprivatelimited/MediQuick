          ...
          defaultArticles.forEach(async (art) => {
            if (!existingIds.has(art.id)) {
              try {
                ...
                console.log(`Auto-seeded missing blog to Firestore (BlogDetails): ${art.title}`);
              } catch (e) {
                console.error("Auto-seeding blog failed in BlogDetails:", e);
              }
            }
          });
        }
    ...
    <div className="bg-[#F8FCFC] min-h-screen py-12 font-sans text-dark/95 text-left">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="bg-white rounded-[32px] border border-dark/5 shadow-soft p-6 sm:p-10 space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2 select-none">
              ...
              {article.verifiedSource && (
                <span className="bg-emerald-50 text-emerald-700 font-extrabold uppercase text-[10px] px-2.5 py-1 rounded-md tracking-wider flex items-center gap-1">
                  <MdCheckCircle className="text-emerald-500" /> Verified Source: {article.verifiedSource}
                </span>
              )}
            </div>
          ...
          {article.verifiedSource && (
            <div className="text-[10px] sm:text-xs text-dark/50 italic border-t border-dark/5 pt-4 select-none">
              Source & References: Medical guidelines and statistics referenced from the <strong>{article.verifiedSource}</strong>.
            </div>
          )}
